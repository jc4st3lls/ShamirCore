...
namespace Crypto
{
    internal sealed class GF256
    {
        ...
        private static byte Mul(byte a, byte b)
        {
            ...
            return EXP[Convert.ToUInt32(LOG[Convert.ToUInt32(a)]) + Convert.ToUInt32(LOG[Convert.ToUInt32(b)])];
        }

        static byte Div(byte a, byte b)
        {
            // multiply by the inverse of b
            return Mul(a, EXP[255 - Convert.ToUInt32(LOG[Convert.ToUInt32(b)])]);
        }

        internal static byte Eval(byte[] p, byte x)
        {
            // Horner's method
            byte result = 0;
        ...
        internal static byte[] Generate(SecureRandom random, int degree, byte x)
        {
        ...
        internal static byte Interpolate(byte[,] points)
        {
            byte x = 0;
            byte y = 0;
